...
def fix_quiz_deposit_addresses():
    ...
    try:
        ...
        logger.info(f"Found {len(quizzes_to_fix)} quizzes with NULL deposit_address. Fixing...")
        
        # Update each quiz
        for quiz in quizzes_to_fix:
            quiz.deposit_address = Config.DEPOSIT_ADDRESS
            logger.info(f"Updated quiz {quiz.id} ({quiz.topic}) with deposit_address: {Config.DEPOSIT_ADDRESS}")
        
        # Commit all changes
        session.commit()
        logger.info(f"Successfully updated {len(quizzes_to_fix)} quiz(s) with deposit addresses.")
        return True
...
if __name__ == "__main__":
    logger.info(f"Starting quiz deposit address fix. Using deposit address: {Config.DEPOSIT_ADDRESS}")
